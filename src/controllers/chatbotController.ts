  ...
  try {
    ...
    if (query === "q_tac_hai") {
      ...
      if (cards.length > 0) {
        ...
        cards.forEach(card => {
          answer += `**${card.name}**\n- ${card.shortDesc}\n- Cảnh báo: ${card.detailedContent[0]}...\n\n`;
        });
      } else {
    ...
    // 2. XỬ LÝ NGƯỜI DÙNG TỰ GÕ CÂU HỎI (TÌM KIẾM TRONG DATABASE)
      ...
      const keyword = query.toLowerCase().replace(/là gì|thế nào|tác hại của|cho tôi biết/g, "").trim();
      ...
      if (flashcards.length > 0) {
        ...
        flashcards.forEach(card => {
          answer += `**${card.name}**\n- ${card.shortDesc}\n- **Tác hại/Biểu hiện:** ${card.detailedContent.join(" ")}\n\n`;
        });
      } else {
