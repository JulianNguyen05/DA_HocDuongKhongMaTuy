...
export default function LawTree({ initialData }: LawTreeProps) {
  ...
  // Chỉ cần duy nhất state này để đẩy vòng tròn sang trái trên PC, và giữ giữa trên Mobile
  ...
  const RADIUS = 300; // Bán kính vòng tròn khi xem chi tiết (Đã được CSS tự động thu nhỏ trên mobile)

  // Lắng nghe xem có phải mobile không để dời vị trí vòng tròn
  useEffect(() => {
    const checkMobile = () => setIsMobile(window.innerWidth < 768);
    checkMobile(); // Check lần đầu
  ...
  return (
    <div className="relative w-full h-full flex justify-center font-sans overflow-hidden bg-[#D1FAE0]">
      <TreeParticles />

      {/* MASTER WRAPPER: Sử dụng origin-top và scale để thu nhỏ toàn bộ cây + quả */}
      <div 
        ref={treeRef} 
        className="absolute top-0 left-1/2 -translate-x-1/2 w-[1200px] h-[900px] origin-top scale-[0.65] sm:scale-[0.75] md:scale-[0.85] lg:scale-90 xl:scale-100 pointer-events-none"
      >
        ...
        <AnimatePresence>
          {activeId && (
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[200vw] h-[200vh] bg-[#D1FAE0]/80 z-20 cursor-pointer pointer-events-auto"
        ...
        <motion.div
          ...
          animate={{
            // Đẩy sang trái 20% trên PC để lấy chỗ cho Modal, giữ ở 50% trên Mobile
            left: activeId ? (isMobile ? "50%" : "20%") : "50%",
            x: "-50%",
        ...
        >
          ...
          {initialData.map((law, index) => {
            ...
            const dist = activeIndex !== -1 ? Math.min(Math.abs(index - activeIndex), TOTAL_LEAVES - Math.abs(index - activeIndex)) : 0;
            const leafScale = activeId !== null ? dist === 0 ? 1.4 : dist === 1 ? 1.0 : dist === 2 ? 0.85 : 0.7 : 1;
            const leafOpacity = activeId !== null ? dist === 0 ? 1 : dist === 1 ? 0.8 : dist === 2 ? 0.5 : 0.2 : 1;
            const zIndexLv = activeId !== null ? isActive ? 50 : 40 : hoveredId === law.id ? 50 : 10;
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  <div className={`relative z-10 w-full h-full rounded-full flex flex-col items-center justify-center shadow-[inset_-5px_-5px_15px_rgba(0,0,0,0.6),3px_5px_10px_rgba(0,0,0,0.4)] transition-all duration-300 ${isActive ? "bg-[radial-gradient(circle_at_35%_30%,#fdba74,#ea580c,#7c2d12)] border border-orange-400" : "bg-[radial-gradient(circle_at_35%_30%,#6ee7b7,#059669,#064e3b)] border border-emerald-400"}`}>
                    <span className={`font-black text-center leading-tight drop-shadow-md text-white ${isActive ? "text-[12px] md:text-[16px]" : "text-[10px] md:text-sm"}`}>
                      {law.article}
                    </span>
                  </div>
